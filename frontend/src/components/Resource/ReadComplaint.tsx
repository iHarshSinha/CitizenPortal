...
import ComplaintModel from '../../models/ComplaintModel';
  ...
  const {data:dataRes,isLoading:isLoadingDataRes,error:errorDataRes}= useQuery({
    queryKey: ['resourceData', 'complaint'],
     queryFn: async () => {
...
const accessToken = getCookie("access_token");
  ...
  if (!accessToken) {
    throw new Error("Access token not found");
      ...
      const response = await fetch(
        `${apiConfig.getResourceUrl('complaint')}?` + params.toString(),
        {
          method: "GET",
          headers: {
  ...
  const {data: dataResMeta,isLoading:isLoadingDataResMeta,error:errorDataResMeta} = useQuery({
    queryKey: ['resourceMetaData', 'complaint'],
   queryFn: async () => {
      const response = await fetch(
        `${apiConfig.getResourceMetaDataUrl('complaint')}?`,
        {
          method: "GET",
  ...
  useEffect(() => {
    if (fetchData && fetchData.length > 0) {
      ...
      const modelObjects = fetchData.map((obj: any) => ComplaintModel.fromJson(obj));
      ...
      console.log("row data fetcheed",jsonObjects)
    }
  }, [fetchData]);
